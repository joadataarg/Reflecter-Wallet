...
export default function DashboardPage() {
    ...
    useEffect(() => {
        const autoSync = async () => {
            // If user is logged in via Firebase but Chipi wallet not found
            if (user && !wallet && !walletLoading && !isSyncing) {
                setIsSyncing(true);
                try {
                    console.log('Auto-syncing wallet for user:', user.uid);
        ...
        autoSync();
    }, [user, wallet, walletLoading, isSyncing, getToken, createWalletAsync, refetch]);
    ...
    if (walletLoading || isSyncing) {
        return (
            <div className="flex-1 flex flex-col items-center justify-center p-8 space-y-4 bg-black">
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-8 text-center space-y-4 bg-black h-full">
                <div className="p-6 bg-red-500/5 border border-red-500/20 rounded-[2rem] max-w-xs">
                    <p className="text-red-500 text-[10px] font-black uppercase tracking-[0.2em] mb-2 leading-relaxed">
                        No se pudo encontrar o vincular tu billetera
                    </p>
                    <p className="text-[9px] text-zinc-600 font-bold uppercase tracking-widest">
                        {walletError?.message || 'Error de conexión con el servidor de orquestación.'}
                    </p>
                </div>
                <button
