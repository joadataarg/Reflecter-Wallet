...
const WalletPopup: React.FC<WalletPopupProps> = ({ isOpen, onClose, isEmbedded = false, initialAuthView = 'login' }) => {
  const { user, signIn, signUp, signOut, getToken, signInWithGoogle } = useFirebaseAuth();
  ...
  const handleGoogleAuth = async () => {
    ...
    try {
      ...
      if (!firebaseUser) throw new Error('Google Sign In cancelled or failed.');
      ...
      // 4. Derive Encryption Key (This will be deterministic for the same Google user)
      const encryptKey = await deriveEncryptKey(userUid);

      // 5. Create Wallet/Login check with ChipiPay
      // If wallet exists, it will just return it. If not, it creates it.
      const response = await createWalletAsync({
        params: {
          encryptKey,
          externalUserId: userUid,
      ...
      console.log('Wallet authenticated/created via Google:', response.txHash);

    } catch (err: any) {
      console.error('Google Auth Error:', err);
      if (err.status === 401) {
        setError('Error de autorización: Verifica la configuración de JWT.');
      } else {
            ...
            <div className="p-8 flex flex-col animate-in fade-in duration-500">
              ...
              <form onSubmit={handleAuthAction} className="space-y-4 flex-1">
                ...
                <div className="relative flex items-center gap-4 my-4">
                  ...
                  <span className="text-[10px] text-zinc-500 uppercase tracking-widest font-bold">O continuar con</span>
                  <div className="h-px bg-white/10 flex-1" />
                </div>
                ...
                <button
                  ...
                  disabled={isLoading}
                  className="w-full py-4 font-bold uppercase tracking-widest text-xs transition-all flex items-center justify-center gap-2 disabled:opacity-50 bg-white/5 text-white hover:bg-white/10 border border-white/10"
                >
